    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-soft max-w-lg w-full max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-saas-border">
          <div>
            <h3 className="text-lg font-bold text-saas-text-primary">Riwayat Aktivitas</h3>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-3">
          ...
          {loading && <p className="text-gray-500 text-sm">Memuat riwayat...</p>}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-400">
                    {log.source === 'MANUAL' ? 'manual' : 'otomatis'}
                  </span>
                </div>
                <p className="text-xs text-gray-500">
                  {format(new Date(log.startedAt), 'dd MMM yyyy, HH:mm')}
                  {log.endedAt ? ` – ${format(new Date(log.endedAt), 'HH:mm')}` : ' – sekarang'}
                </p>
              </div>
